...
public class ScramSession {
  ...
  /**
   * Constructs a SCRAM client, to perform an authentication for a given user. This class can be
   * instantiated directly, but it is recommended that a {@link ScramClient} is used instead.
   *
   * @param scramMechanism The SCRAM mechanism that will be using this client
   * @param stringPreparation used for the session
   * @param user used for the session
   * @param nonce used for the session
   */
  public ScramSession(ScramMechanism scramMechanism, StringPreparation stringPreparation,
      String user, String nonce) {
  ...
  /**
   * Returns the text representation of a SCRAM client-first-message, with the GSS-API header values
   * indicated.
   *
  ...
  public String clientFirstMessage(Gs2CbindFlag gs2CbindFlag, String cbindName, String authzid) {
    return setAndReturnClientFirstMessage(
        new ClientFirstMessage(gs2CbindFlag, authzid, cbindName, user, nonce));
  }

  /**
   * Returns the text representation of a SCRAM client-first-message, with no channel binding nor
   * authzid.
   *
   * @return The message
   */
   ...
   * {@link #receiveServerFirstMessage(String)}.
   */
  public class ServerFirstProcessor {
     ...
     * Generates a {@link ClientFinalProcessor}, that allows to generate the client-final-message
     * and also receive and parse the server-first-message. It is based on the user's password.
     *
     * @param password The user's password
     * @return The handler
     * @throws IllegalArgumentException If the message is null or empty
    ...
    /**
     * Generates a {@link ClientFinalProcessor}, that allows to generate the client-final-message
     * and also receive and parse the server-first-message. It is based on the clientKey and
     * storedKey, which, if available, provide an optimized path versus providing the original
     * user's password.
     *
  ...
  public class ClientFinalProcessor {
    ...
    private ClientFinalProcessor(String nonce, byte[] clientKey, byte[] storedKey,
        byte[] serverKey) {
      assert null != clientKey : "clientKey";
      assert null != storedKey : "storedKey";
      assert null != serverKey : "serverKey";
    ...
    private ClientFinalProcessor(String nonce, String password, String salt, int iteration) {
      this(
          nonce,
          ScramFunctions.saltedPassword(
              scramMechanism, stringPreparation, password, Base64.getDecoder().decode(salt),
              iteration));
    }
    ...
    private synchronized void generateAndCacheAuthMessage(byte[] cbindData) {
      ...
      authMessage = clientFirstMessage.writeToWithoutGs2Header(new StringBuffer())
          ...
          .append(serverFirstMessageString)
          .append(",")
          .append(ClientFinalMessage.writeToWithoutProof(clientFirstMessage.getGs2Header(),
              cbindData, nonce))
          .toString();
    }
     ...
     * @throws ScramServerErrorException If the server-final-message contained an error
     * @throws IllegalArgumentException If the message is null or empty
     * @throws ScramInvalidServerSignatureException If server SCRAM signature is invalid
     */
    public void receiveServerFinalMessage(String serverFinalMessage)
        throws ScramParseException, ScramServerErrorException, ScramInvalidServerSignatureException,
        IllegalArgumentException {
