 ...
 * }
 *
 * <p>Note that extensions are not supported (and, consequently, error message extensions).
 *
...
public class ServerFinalMessage implements StringWritable {
  ...
  public enum Error {
    ...
    private static Map<String, Error> valuesAsMap() {
      return Arrays.stream(values())
          .collect(Collectors.toMap(Error::getErrorMessage, Function.identity()));
    }
  ...
  public StringBuffer writeTo(StringBuffer sb) {
    return StringWritableCsv.writeTo(
        sb,
        isError() ? new ScramAttributeValue(ScramAttributes.ERROR, error.errorMessage)
