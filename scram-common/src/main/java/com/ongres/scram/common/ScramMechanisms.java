 ...
 * by the Java JDK version 6 or greater.
 *
 * {@link java.security.MessageDigest}: "Every implementation of the Java platform is required to
 * support the following standard MessageDigest algorithms: {@code MD5}, {@code SHA-1},
 * {@code SHA-256}".
 *
 * {@link javax.crypto.Mac}: "Every implementation of the Java platform is required to support the
 * following standard Mac algorithms: {@code HmacMD5}, {@code HmacSHA1}, {@code HmacSHA256}".
 *
...
public enum ScramMechanisms implements ScramMechanism {
  ...
  ScramMechanisms(String name, String hashAlgorithmName, int keyLength, String hmacAlgorithmName,
      boolean channelBinding, int priority) {
  ...
  public byte[] digest(byte[] message) {
    try {
      ...
      throw new ScramRuntimeException(
          "Algorithm " + hashAlgorithmName + " not present in current JVM");
    }
  }
  ...
  public byte[] hmac(byte[] key, byte[] message) {
    try {
      ...
      throw new ScramRuntimeException(
          "MAC Algorithm " + hmacAlgorithmName + " not present in current JVM");
    }
  }
  ...
  public byte[] saltedPassword(StringPreparation stringPreparation, char[] password, byte[] salt,
    ...
    try {
      ...
      throw new ScramRuntimeException("Unsupported PBKDF2 for " + mechanismName);
    }
  }
   ...
   * @return An Optional instance that contains the ScramMechanism if it was found, or empty
   *         otherwise.
   */
   ...
   *
   * <p>Given a list of SCRAM mechanisms supported by the peer, pick one that matches the channel
   * binding requirements and has the highest priority.
   *
   ...
   */
  public static ScramMechanism selectMatchingMechanism(boolean channelBinding,
